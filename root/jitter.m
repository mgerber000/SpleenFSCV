function [x,yn,idxn] = jitter(y,thresh,minx)

[gr,id] = qtcluster([zeros(length(y),1),zeros(length(y),1),y],thresh);
x = [];
yn = [];
idxn = [];
for i = 1:length(gr)
    count = length(gr{1,i}(:,3));
    x = [x;linspace(1 - minx*(count-1)/2,1 + minx*(count-1)/2,count)'];
    yn = [yn;gr{1,i}(:,3)];
    idxn = [idxn;id{1,i}];
end
...
function [MEP,index] = qtcluster(MEP,thresh)
